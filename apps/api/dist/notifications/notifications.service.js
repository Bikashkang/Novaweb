...
const appointment_created_template_1 = require("./templates/appointment-created.template");
const appointment_status_changed_template_1 = require("./templates/appointment-status-changed.template");
const payment_confirmed_template_1 = require("./templates/payment-confirmed.template");
const prescription_created_template_1 = require("./templates/prescription-created.template");
const video_call_ready_template_1 = require("./templates/video-call-ready.template");
const appointment_reminder_template_1 = require("./templates/appointment-reminder.template");
let NotificationsService = NotificationsService_1 = class NotificationsService {
    ...
    async logNotification(recipientEmail, notificationType, status, errorMessage) {
        if (!this.supabase)
            return;
        try {
    ...
    async sendAppointmentCreated(dto) {
        if (!this.resend) {
        ...
        try {
            ...
            const html = (0, appointment_created_template_1.getAppointmentCreatedTemplate)({
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f5f5f5;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f5f5f5; padding: 20px;">
    ...
    async sendAppointmentStatusChanged(dto) {
        if (!this.resend) {
            this.logger.warn('Resend not configured. Skipping email notification.');
            return;
        }
        try {
            ...
            const html = (0, appointment_status_changed_template_1.getAppointmentStatusChangedTemplate)({
                patientName: dto.patientName,
                doctorName: dto.doctorName,
                appointmentDate: dto.appointmentDate,
                appointmentTime: dto.appointmentTime,
    ...
    async sendPaymentConfirmed(dto) {
        if (!this.resend) {
            this.logger.warn('Resend not configured. Skipping email notification.');
            return;
        }
        try {
            const html = (0, payment_confirmed_template_1.getPaymentConfirmedTemplate)({
                patientName: dto.patientName,
                doctorName: dto.doctorName,
                appointmentDate: dto.appointmentDate,
                appointmentTime: dto.appointmentTime,
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f5f5f5;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f5f5f5; padding: 20px;">
    ...
    async sendPrescriptionCreated(dto) {
        if (!this.resend) {
            this.logger.warn('Resend not configured. Skipping email notification.');
            return;
        }
        try {
            ...
            const html = (0, prescription_created_template_1.getPrescriptionCreatedTemplate)({
                patientName: dto.patientName,
                doctorName: dto.doctorName,
    ...
    async sendVideoCallReady(dto) {
        ...
        try {
            ...
            const html = (0, video_call_ready_template_1.getVideoCallReadyTemplate)({
                patientName: dto.patientName,
                doctorName: dto.doctorName,
                appointmentDate: dto.appointmentDate,
                appointmentTime: dto.appointmentTime,
    ...
    async sendAppointmentReminder(dto) {
        if (!this.resend) {
            this.logger.warn('Resend not configured. Skipping email notification.');
            return;
        }
        try {
            const html = (0, appointment_reminder_template_1.getAppointmentReminderTemplate)({
                patientName: dto.patientName,
                doctorName: dto.doctorName,
                appointmentDate: dto.appointmentDate,
                appointmentTime: dto.appointmentTime,
